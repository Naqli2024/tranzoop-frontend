...
export default function Ledger() {
  ...
  const handleViewSupplierLedger = async (supplier) => {
    try {
      ...
      const res = await dispatch(
        getSupplierLedgerById(supplier.supplierId),
      ).unwrap();
  ...
  const filteredSuppliers = Array.isArray(supplierData)
    ? supplierData.filter((s) => {
        const matchesSearch = s.supplierName
          ?.toLowerCase()
          .includes(searchCustomer.toLowerCase());
        ...
        <div className="pl-tab-body" style={{ overflowY: "auto" }}>
          ...
          >
            <div className="pl-mc" style={{ flex: 1, minWidth: 120 }}>
              <div className="pl-mc-l d-flex align-items-center">
                Today Sales{" "}
                <span style={{ fontSize: "8px" }}>(without GST)</span>
              </div>
              <div className="pl-mc-v">
                ₹
                {summaryData?.todaySales.toLocaleString("en-IN", {
            ...
            <div className="pl-mc" style={{ flex: 1, minWidth: 120 }}>
              ...
              <div className="pl-mc-v">
                ₹
                {summaryData?.totalReceived.toLocaleString("en-IN", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                }) || 0}
              </div>
            ...
            <div className="pl-mc" style={{ flex: 1, minWidth: 120 }}>
              ...
              <div className="pl-mc-v">
                ₹
                {summaryData?.supplierPayable.toLocaleString("en-IN", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                }) || 0}
              </div>
          ...
          <div style={{ padding: "0 14px 60px" }}>
            <div className="pl-section mt-3">
              <div className={`pl-section-hdr pl-hdr-sales`}>
                Daily Sales — ₹
              ...
              <div className="pl-body">
                {Array.isArray(tSalesData) &&
                  tSalesData.map((sale, index) => (
            ...
            <div className="pl-section mt-3">
              <div className={`pl-section-hdr pl-hdr-income`}>
                Aging Report — ₹
                {summaryData?.totalOutstanding.toLocaleString("en-IN") || 0}
              </div>
              <div className="pl-body">
                {agingData &&
            ...
            <div className="pl-section mt-3">
              <div className={`pl-section-hdr pl-hdr-expenses`}>
                Revenue Split — {totalRevenue.toLocaleString("en-IN") || 0}
              </div>
              <div className="pl-body">
                {revenueData &&
                  Object.entries(revenueData).map(([range, amount]) => (
                    <div key={range} className="pl-row">
              ...
              <div className="ledger-pay-grid">
                {paymentData &&
                  Object.entries(paymentData).map(([key, value]) => (
                    ...
                    >
                      <div className="ledger-pay-icon">{paymentIcons[key]}</div>
                      <div className="ledger-pay-label">
                        {key.toUpperCase()}
                      </div>
                    ...
                    {filteredCustomers.length > 0 ? (
                      filteredCustomers.map((c) => (
                        <tr key={c.customerId}>
                          ...
                          <td>
                            ₹
                            {c.totalBill.toLocaleString("en-IN", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </td>
                    ...
                    {filteredSuppliers.length > 0 ? (
                      filteredSuppliers.map((s) => (
                        <tr key={s.supplierId}>
                          ...
                          <td>
                            ₹
                            {s.totalPurchase.toLocaleString("en-IN", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </td>
